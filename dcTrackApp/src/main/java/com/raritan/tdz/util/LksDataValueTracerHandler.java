 ...
 *
 */
public class LksDataValueTracerHandler implements ObjectTracerHandler {
	...
	public void handleTrace(ObjectTracer tracer, Class<?> rootClass,
			String fieldName) {
		String lksFieldName = fieldName.split("LkpValue")[0] + "Lookup";
		...
		if (traceFields.size() > 0){
			...
			tracer.appendField(lksValueTracer.traceObject(LksData.class, "lkpValue").get(0));
		}		
	}
