...
public abstract class ValueIdToDomainAdaptorTemplate implements ValueIDToDomainAdaptor {
	...
	private final Logger log = Logger.getLogger(ValueIdToDomainAdaptorTemplate.class);
	
	@Autowired
	private FieldHome fieldHome;
	
	public ValueIdToDomainAdaptorTemplate( RulesProcessor processor, RemoteRef remoteRef,
	...
	@Override
	public Object convert(Object target, List<ValueIdDTO> valueIdDTOList,
			String unit) throws BusinessValidationException,
			IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, ClassNotFoundException,
			DataAccessException {
		Map<String, String> errorMap = new HashMap<String, String>();
		validationErrors = new MapBindingResult(errorMap, target.getClass().getName());

		processValueIdDtoList( target, valueIdDTOList, unit);
		specialProcessValueIdDtoList(target, valueIdDTOList, unit);
		//changeCodeToUpperCase((DataCenterLocationDetails)location);
		
		return target;
	}

	protected abstract Object specialProcessValueIdDtoList(Object target,
			List<ValueIdDTO> valueIdDTOList, String unit);
	
	protected abstract String getSpecialUiId();
	
	protected abstract boolean isSkip(String uiId);
	...
	private void processValueIdDtoList (Object target, List<ValueIdDTO> valueIdDTOList, String unit)
		...
		for (ValueIdDTO dto: valueIdDTOList){
 			...
 			if (isSkip(dto.getLabel())) continue;
			...
			if (remoteId != null && !dto.getLabel().equals(getSpecialUiId())){
			...
			setValue( target, trace, value);
		}
		setDefaultValues(target);
	}
	

	private void setDefaultValues(Object target) throws ClassNotFoundException{
