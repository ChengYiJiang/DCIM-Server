...
public class CabElevBackImgHandler extends ImageEventAdapter {
	...
	public void onCreate(IImageInstance image, IReportContext reportContext) {
		try {
			...
			if (image.getRowData().getColumnValue("model_id") != null) {
				...
				ImageProviderProxy provider = (ImageProviderProxy) reportContext.getAppContext().get(ReportConstants.CABELEBACKIMAGEPROVIDERPROXY_ID);
