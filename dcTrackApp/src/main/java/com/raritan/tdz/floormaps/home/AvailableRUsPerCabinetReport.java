 ...
 * @author Randy Chen
 */
public class AvailableRUsPerCabinetReport extends ReportHomeBase implements ReportHome {

	public AvailableRUsPerCabinetReport(SessionFactory sessionFactory ){
		super(sessionFactory);
		log.info("AvailableRUsPerCabinetReport init...");
		
		defaultColorSchema.put(LOW,"FF0000");
		defaultColorSchema.put(MEDIUM,"FFFF00");
		defaultColorSchema.put(HIGH,"00FF00");
	...
	public ReportDataDTO getReportData(String reportType,String locationCode,String filterType,String filterValue) throws DataAccessException {
		...
		String[][] columns={
			{"column0","itemId"},
			{"column1","Cabinet"},
			{"column2","Available RUs"},
			{"color","Color"},
			{"cadHandle","CadHandle"}
		};
			
		String querySQL=
			"SELECT items.item_id  AS itemId, items.item_name AS Cabinet,free_ru AS RUs,items.cad_handle AS cadHandle "+
		...
		String[] orderArray={LOW, MEDIUM, HIGH, NO_DATA};
