...
public class DataConnBeanAddTransformer extends DataConnBeanTransformer {
	public DataConnBeanAddTransformer(String uuid) {
		super(uuid);
	}
	...
	public Object[] convert(DCTImport beanObj, Errors errors) throws DataAccessException, Exception {
		...
		if (pc == null) {
			...
			//check again if there are an existing circuit using the port id - needed to cover case when circuit is reserved.
			if(circuit != null) {
