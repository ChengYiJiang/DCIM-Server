 ...
 *
 */
public class TicketPowerPortQuantityMethodCallback implements
		...
		if (itemIdStr != null && !itemIdStr.isEmpty()){
			...
			if (ticketIds != null && ticketIds.size() > 0) {
				...
				if (origPorts != null && origPorts.size() > 0){
					Integer qty = origPorts.get(0).getQuantity();
					
					uiValueIdField.setValue(qty);
					uiValueIdField.setValueId(qty);
				} else {
					Integer qty = 0;
					uiValueIdField.setValue(qty);
					uiValueIdField.setValueId(qty);
