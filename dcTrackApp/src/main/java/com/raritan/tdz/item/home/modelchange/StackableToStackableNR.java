 ...
 *
 */
public class StackableToStackableNR implements ChangeModel {

	ChangeModelDAO changeModelDAO;
	Logger log = Logger.getLogger(ChangeModel.class);
	
	public StackableToStackableNR(ChangeModelDAO changeModelDAO) {
	...
	public void change(Item itemInDB, Item itemToSave)
		...
		log.debug("Standard To Standard NR");
		
		//1
		changeModelDAO.updateUPosition(itemInDB, itemToSave, UPositionEnum.KEEP_UPOSITION);
		
		//3 -Update Shelf position
		changeModelDAO.updateShelfPosition(itemToSave);
	}
