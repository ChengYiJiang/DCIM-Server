 ...
 *
 */
public class TicketPowerSupplyRedundancyMethodCallback implements
	...
	public void fillValue(UiComponent uiViewComponent,
		...
		//Dont fill anything if this is for a model. We need to support this only for items
		if (filter.toSqlString().contains("model")) return;
		...
		if (itemIdStr != null && !itemIdStr.isEmpty()){
			...
			if (ticketIds != null && ticketIds.size() > 0) {
				...
				if (origPorts != null && origPorts.size() > 0){
					String redundancy = origPorts.get(0).getPsRedundancy();
					
					uiValueIdField.setValue(redundancy);
					uiValueIdField.setValueId(redundancy);
				}
			}
		}
