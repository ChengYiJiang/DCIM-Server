 ...
 *
 */
public class DataConnVmClusterValidator implements Validator {
	
	@Autowired
	DataPortDAO dataPortDAO;

	public DataConnVmClusterValidator() {
		super();
	}
	/* (non-Javadoc)
	...
	public void validate(Object target, Errors errors) {
		...
		if(!sPort.isVirtual()) return;

		ItItem item =(ItItem)sPort.getItem();
		long sPortClusterId = item.getVmClusterLookup() == null ? 0: item.getVmClusterLookup().getLkuId();

		item =(ItItem)dPort.getItem();
		long dPortClusterId = item.getVmClusterLookup() == null ? -1: item.getVmClusterLookup().getLkuId();
		
		if(sPortClusterId != dPortClusterId){
			String errMsg = ApplicationCodesEnum.DATA_CIR_VM_CLUSTER.value();
