 ...
 *
 */
public class DataConnPhys2LogOrVmValidator implements Validator {
	
	@Autowired
	DataPortDAO dataPortDAO;

	public DataConnPhys2LogOrVmValidator() {
		super();
	}
	/* (non-Javadoc)
	...
	public void validate(Object target, Errors errors) {
		...
		if(!sPort.isPhysical()) return;
		
		if(dPort.isVirtual() || dPort.isLogical()){
			String errMsg = ApplicationCodesEnum.DATA_CIR_PHYSICAL_LOG_OR_VM.value();
