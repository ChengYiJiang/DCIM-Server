 ...
 *
 */
public class BladeItemWithConnectionsLocationValidatorTest extends UnitTestBase {
	  ...
	  public void testUnusedPortsOnItItem(){
		...
		//  powerPorts.addAll(item.getPowerPorts());
		 ...
		 // sensorPorts.addAll(item.getSensorPorts());
		  ...
		  bladeItemWithConnectionsValidator.validate(getTargetMap(item), errors);
	  ...
	  public void testUsedSensorPortsOnItItem(){
		 ...
		 // powerPorts.addAll(item.getPowerPorts());
		  ...
		  //This is because we do not check with sensors for blades as blades do not support sensors (atleast not yet!)
		  Assert.assertFalse(errors.hasErrors());
	  }
	  ...
	  public void testUsedAllTypeOfPortsOnItItemWithCabinetFilled(){
		  ...
		  for (ObjectError error:errors.getAllErrors()){
			  Assert.assertTrue((error.getArguments()[1]).toString().contains("Chassis"));
			  Assert.assertTrue((error.getArguments()[1]).toString().contains("Slot"));
		  }
	  }
	  
	  @Test
	  public void testUsedAllTypeOfPortsOnItItemWithChassisFilled(){
		  ItItem item = createItItemWithUnusedPorts();
		  item.setBladeChassis(new ItItem());
	  ...
	  @Test
	  public void testUsedAllTypeOfPortsOnItItemWithCabinetUPositionAndChassisFilled(){
		  ItItem item = createItItemWithUnusedPorts();
		  item.setParentItem(new Item());
		  item.setBladeChassis(new ItItem());
		  item.setuPosition(10);
	  ...
	  @Test
	  public void testUsedAllTypeOfPortsOnItItemWithEverythingFilled(){
		  ItItem item = createItItemWithUnusedPorts();
		  item.setParentItem(new Item());
		  item.setBladeChassis(new ItItem());
		  item.setuPosition(10);
		  item.setSlotPosition(1);
